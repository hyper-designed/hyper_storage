...
void main() {
  ...
  group('HiveBackend', () {
    late HiveBackend backend;
    late String boxName;

    setUp(() async {
      boxName = uniqueBoxName();
      backend = HiveBackend(boxName: boxName);
      await backend.init();
    });
    ...
    group('initialization', () {
      test('creates backend with default box name', () async {
        final defaultBackend = HiveBackend();
      ...
      test('creates backend with custom box name', () {
        final customName = uniqueBoxName('custom');
        final customBackend = HiveBackend(boxName: customName);
        expect(customBackend.name, customName);
      });

      test('throws ArgumentError when boxName is "default"', () {
        expect(
          () => HiveBackend(boxName: 'default'),
          throwsArgumentError,
        );
      });
      ...
      test('box getter throws StateError before init', () {
        final uninitializedBackend = HiveBackend(boxName: uniqueBoxName());
      ...
      test('init opens the box correctly', () async {
        final newBackend = HiveBackend(boxName: uniqueBoxName());
      ...
      test('can create multiple backends with different names', () async {
        final backend1 = HiveBackend(boxName: uniqueBoxName('b1'));
        final backend2 = HiveBackend(boxName: uniqueBoxName('b2'));
    ...
    group('container support', () {
      test('container() creates new backend with given name', () async {
        final containerName = uniqueBoxName('container');
      ...
      test('multiple containers work independently', () async {
        final container1 = await backend.container(uniqueBoxName('c1'));
        final container2 = await backend.container(uniqueBoxName('c2'));
