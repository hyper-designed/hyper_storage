...
void main() {
  group('SharedPreferencesBackend', () {
    late SharedPreferencesBackend backend;
    late MockSharedPreferencesAsync mockPrefs;

    setUp(() async {
      mockPrefs = MockSharedPreferencesAsync();
      backend = SharedPreferencesBackend(mockPrefs);
      await backend.init();
    });
    ...
    group('initialization', () {
      test('creates backend with SharedPreferencesAsync', () {
        expect(backend, isA<SharedPreferencesBackend>());
      });

      test('init completes successfully', () async {
        final newPrefs = MockSharedPreferencesAsync();
        final newBackend = SharedPreferencesBackend(newPrefs);
        await expectLater(newBackend.init(), completes);
        await newBackend.close();
      });

      test('prefs getter works when SharedPreferencesAsync provided to constructor', () {
        final newPrefs = MockSharedPreferencesAsync();
        final newBackend = SharedPreferencesBackend(newPrefs);
        // When prefs is provided via constructor, it's available immediately
        expect(() => newBackend.prefs, returnsNormally);
        expect(newBackend.prefs, same(newPrefs));
      });

      test('prefs getter works after init', () async {
        expect(() => backend.prefs, returnsNormally);
        expect(backend.prefs, isA<MockSharedPreferencesAsync>());
      });

      test('can create backend with custom SharedPreferencesAsync instance', () {
        final customPrefs = MockSharedPreferencesAsync();
        final customBackend = SharedPreferencesBackend(customPrefs);
        expect(() => customBackend.prefs, returnsNormally);
      });
    });
    ...
    group('int operations', () {
      ...
      test('setInt handles large numbers', () async {
        await backend.setInt('large', 9223372036854775807); // max int64
    ...
    group('lifecycle', () {
      test('close clears prefs reference', () async {
        await backend.close();
        expect(() => backend.prefs, throwsStateError);
      });
